...
% Simple script to perform ULM on in vivo data IQ are loaded, filtered and sent 
% into ULM processing. Bubbles are detected, selected, then localized, and paired 
% into tracks. The result is a list of interpolated tracks to reconstructed a 
% full brain vascularization.
% 
% This code can be easily adapted for any other in vivo datasets.
% 
% Created by Arthur Chavignon 25/02/2020
% 
% DATE 2020.12.17 - VERSION 1.1 AUTHORS: Arthur Chavignon, Baptiste Heiles, 
% Vincent Hingot. CNRS, Sorbonne Universite, INSERM. Laboratoire d'Imagerie Biomedicale, 
% Team PPM. 15 rue de l'Ecole de Medecine, 75006, Paris Code Available under Creative 
% Commons Attribution-NonCommercial-ShareAlike 4.0 International (see <https://creativecommons.org/licenses/by-nc-sa/4.0/ 
% https://creativecommons.org/licenses/by-nc-sa/4.0/>) ACADEMIC REFERENCES TO 
% BE CITED Details of the code in the article by Heiles, Chavignon, Hingot, Lopez, 
% Teston and Couture. Performance benchmarking of microbubble-localization algorithms 
% for ultrasound localization microscopy, Nature Biomedical Engineering, 2021. 
% General description of super-resolution in: Couture et al., Ultrasound localization 
% microscopy and super-resolution: A state of the art, IEEE UFFC 2018
...
workingdir = [workingdir '/Example/IQ_' num2str(downsample_x)]
mkdir(workingdir);
...
% A few parameters must be provided by the user depending of your input images 
% (size of pixel, wavelength) These parameters will be copied and used later during 
% the creation of the ULM structure.
...
framerate = double(UF.FrameRateUF) / downsample_x;          % imaging framerate in [Hz]
...
clear P UF IQ
%% ULM parameters
% this script can be run using different scaling, it can be wavelength, pixel 
% size, mm, um. In this example, input pixel are isotropic and equal to lambda 
% (pixelPitch_x = pixelPitch_y = lambda) All size defined later are expressed 
% in lambda
...
ULM.ButterCuttofFreq = [50/downsample_x 249/downsample_x];        % Cutoff frequency (Hz) for additional filter. Typically [20 300] at 1kHz.
...
% for hhh = 1:Nbuffers
    ...
    tmp.IQ = tmp.IQ(:, :, 1:downsample_x:800);
...
% By cutting Tracks into different variables small than 2GB, the save v6 is 
% faster than save v7.3
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Create MatOut %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% create the MatOut density with interpolated tracks for visual analysis, and 
% with non interpolated tracks for aliasing index calculation. Define the size 
% of SRpixel for displaying (default 10)
