...
for sr in range(0,len(sample_rate_list)):
    ...
    mass_matrix = np.load("mass_matrix_{}_gauss.npy".format(sample_rate))
    _,t,q = data.shape
    ...
    os.makedirs("IQ_{}_gauss".format(sample_rate),exist_ok=True)
    for idx in tqdm(range(1,241)):
        ...
        with open("IQ_{}_gauss/loss_dic_{}.json".format(sample_rate,idx),"w") as f:
            json.dump(loss_dic,f,indent=4)
        np.save("IQ_{}_gauss/my_result_{}.npy".format(sample_rate,idx),interpolated_data)
